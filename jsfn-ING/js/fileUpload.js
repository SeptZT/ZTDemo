 ...
 * @param   drag         [boolean, 拖拽上传（可开启和关闭）]
 * @param   progress     [boolean, 进度条（可开启和关闭）]
 * @param   successTips  [boolean, 上传完成提示（可开启和关闭）]
 * @param   onSuccess    [function, 上传完成回调（可自定义）]
 * @param   onError      [function, 错误处理（可自定义）]
 * @param   mimeTypes    [array, 文件类型限制（可配置）]
 * @param   size         [number, 单文件大小限制（可配置）,（单位KB）]
 * @param   maximum      [number, 可同时上传的最大个数]
 * */
